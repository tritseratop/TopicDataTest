...
template <class T>
bool operator==(const DataCollection<std::vector<T>>& lhs,
				const DataCollection<std::vector<T>>& rhs)
{
	bool res = false;
	if (lhs.value.size() == rhs.value.size())
	{
		...
		{
			res = std::equal(
				lhs.value[i].begin(), lhs.value[i].end(), rhs.value[i].begin(), rhs.value[i].end());
			if (res = false)
				break;
		}
	}
